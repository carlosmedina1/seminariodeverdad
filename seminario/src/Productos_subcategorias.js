...
export default function Productos_propios({ navigation }) {
    ...
    const subcategoria = navigation.getParam('subcategoria', '0')

    const filtrarReport = (text) => {
        //FILTRO DE PRODUCTOS EN LAS SUBCATEGORIAS
        if (text) {
    ...
    const getReports = async () => {
        //OBTIENE LOS PRODUCTOS DE SUBCATEGORIA ESPECIFICA
        setBusqueda('')
        try {
            setLoading(true)
            const json = JSON.stringify({ id_subcategoria: subcategoria.id_subcategoria })
            const response = await fetch(Route + 'busquedaProductosSubcategoria',
                {
                    method: 'POST',
                    headers: {
