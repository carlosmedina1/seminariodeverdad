...
export default function ListadoReporteProductos({ navigation }) {
    ...
    const getReports = async () => {
        //OBTIENE LOS REPORTES DE PRODUCTOS HECHOS POR EL USUARIO
        try {
            setLoading(true)
            const id_user = await AsyncStorage.getItem('id_user')
            const json = JSON.stringify({ id_usuario: id_user})
            const response = await fetch(Route + 'obtenerReportesUsuario',
                {
                    method: 'POST',
                    headers: {
                            ...
                            <View style={{ paddingTop: 20, paddingLeft: 20, paddingRight: 20, justifyContent: 'center' }}>
                                <View style={{ flexDirection: 'column', height: '7%' }}>
                                    <View style={{ flexDirection: 'row', width: '100%', marginBottom: 5, marginTop: 20 }}>
                                        ...
                                        <Text style={{ fontSize: 30, fontWeight: 'bold', color: '#000', }}>Reportes de productos</Text>
                                    </View>
                                </View>
                                <View style={{ flexDirection: 'column', height: '93%' }}>
                                <Text style={{ fontSize: 14, fontWeight: 'bold', color: 'gray', marginTop: 10, }}>Tus reportes de Productos </Text>
                                    {
                                                    ...
                                                    <TouchableOpacity style={styles.itemContainer}>
                                                        <View style={{ flexDirection: 'row', width: '100%' }}>
                                                            ...
                                                            <View style={{ flexDirection: 'column', width: '20%', marginTop: 10 }}>
                                                                {item.vigente ? (
                                                                    <MaterialIcons name="error" color="yellow" size={50} style={{ flex: 1, alignSelf: 'center' }} />
                                                                ):item.aceptado ?(
                                                                    <MaterialIcons name="check" color="green" size={50} style={{ flex: 1, alignSelf: 'center' }} />
                                                                ):(
                                                                    <MaterialIcons name="do-not-disturb" color="red" size={50} style={{ flex: 1, alignSelf: 'center' }} />
                                                                )}
                                                                
                                                            </View>
                                                        </View>
                                                        <Text style={{ flex: 15, fontSize: 17, fontWeight: 'bold', color: '#000', marginTop: 6 }}>{item.justificacion}</Text>
                                                    </TouchableOpacity>
                                                )}
                                                style={{ height: '70%' }}
                                            />
                                            ...
                                            loading ? (
                                                ...
                                                sinProductos ? (
                                                    <View style={{ width: '100%', height: '80%', alignItems: 'center', justifyContent: 'center' }}>
                                                        ...
                                                        <Text style={{ color: '#000', fontSize: 20, fontWeight: 'bold' }}>¡No tienes reportes vigentes!</Text>
                                                    </View>
                                                ) : (
                                                    <View style={{ width: '100%', height: '90%', alignItems: 'center', justifyContent: 'center' }}>
                                                        <Animatable.View animation="tada" easing="ease-out" iterationCount="infinite">
