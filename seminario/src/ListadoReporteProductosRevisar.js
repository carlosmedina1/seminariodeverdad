...
import { View, StyleSheet, Text, ActivityIndicator, ScrollView, TextInput, StatusBar, FlatList, Image, TouchableOpacity, Dimensions, Alert, ToastAndroid, Platform } from 'react-native'
...
export default function ListadoReporteProductos({ navigation }) {
    ...
    const goLista = (item) => {
        //ACCEDE AL DETALLE DEL REPORTE DEL PRODUCTO
        navigation.navigate('ListadoReportesRealesProducto', {
            datax: item,
        })
    }

    const getReports = async () => {
        //OBTIENE LOS REPORTES DE LOS PRODUCTOS
        try {
            setLoading(true)
            const response = await fetch(Route + 'obtenerReportesProducto',
                {
                    method: 'POST',
                    headers: {
                        ...
                        <View style={{ flexDirection: 'column', height: '93%' }}>
                            {
                                filterReport.length > 0 ? (
                                    <FlatList
                                        ...
                                        renderItem={({ item }) => (
                                            <TouchableOpacity style={styles.itemContainer} onPress={() => goLista(item)}>
